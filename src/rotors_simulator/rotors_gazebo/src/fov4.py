...
def listener():
    ...
    # run simultaneously.
    pub = rospy.Publisher('/firefly_4/fov_adj', numpy_msg(Floats),queue_size=10)
    rospy.init_node('fov4', anonymous=True)
    ...
    rospy.Subscriber("/firefly_1/ground_truth/pose", Pose, pos_4)
    rospy.Subscriber("/firefly_4/ground_truth/pose", Pose, fov) 
    #########################################################################################
